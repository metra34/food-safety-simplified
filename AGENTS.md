...
- **Routes live in the `(site)` route group:** All page routes go under `app/(site)/` (e.g. `app/(site)/legal/page.tsx` → `/legal`, `app/(site)/page.tsx` → `/`). The parentheses keep `(site)` out of the URL. Only app-wide files stay at the root of `app/`: `layout.tsx`, `not-found.tsx`, `globals.css`, and `assets/`. Import shared assets with the `@/app/assets/...` alias rather than relative paths.
- **Route-specific components:** Components used by only one route live next to it in `app/(site)/{route}/components/{component-name}.tsx` (e.g. `app/(site)/legal/components/legal-section.tsx`). Move them to `components/{typeOfComponent}/` once a second route needs them.
